...
<div id="real-content">
<style>
    /* Reset & Typography Standar - SAMA dengan index.blade.php */
    .dashboard-container {
        font-family: 'Inter', sans-serif;
    }

    /* Style Kartu identik dengan index.blade.php */
    .card-standard {
        background: white;
    ...
    /* Filter Button Standard - SAMA dengan index */
    .filter-btn {
        padding: 0.5rem 1rem;
    ...
    .filter-btn.active {
        background: #2563eb; /* Blue-600 - SAMA dengan index */
        color: white;
        border-color: #2563eb;
    }
    ...
    /* Badge Insight identik dengan index.blade.php */
    .insight-badge {
                                ...
                                <tbody class="divide-y divide-gray-100 text-xs">
                                    @forelse($latestData ?? $allData ?? [] as $data)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-6 py-4 uppercase font-bold text-blue-600">{{ $data->commodity_name ?? $data->commodity }}</td>
                                            <td class="px-6 py-4 text-gray-500">{{ $data->date }}</td>
                                            <td class="px-6 py-4 font-bold text-emerald-600">Rp {{ number_format($data->price, 0, ',', '.') }}</td>
            ...
            <div class="card-standard p-6">
                ...
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100 mb-6">
                    <div>
                        ...
                        <p class="text-lg text-blue-600 font-bold">{{ count($users ?? []) }} Aktif</p>
                    </div>
                ...
                <div class="p-5 border border-gray-200 rounded-xl">
                    ...
                    <form action="{{ route('admin.storeUser') }}" method="POST" class="space-y-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" required class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-xs text-gray-900 font-medium outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-700 uppercase mb-1.5 block tracking-tight">Alamat Email</label>
                            <input type="email" name="email" required class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-xs text-gray-900 font-medium outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-gray-700 uppercase mb-1.5 block tracking-tight">Kata Sandi</label>
                            <input type="password" name="password" required class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-xs text-gray-900 font-medium outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <button type="submit" class="w-full bg-orange-500 text-white py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-orange-600 transition-all">
                            Buat Pengguna
                        </button>
                    </form>
                </div>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm text-blue-600 font-bold">{{ $user->name }}</span>
                                    <span class="text-xs text-gray-400 font-medium">{{ $user->email }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider {{ $user->is_admin ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600' }}">
                ...
                <div class="p-5 border border-gray-200 rounded-xl">
                    ...
                    <form action="{{ route('admin.storeUser') }}" method="POST" class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="role" required class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-xs text-gray-900 font-medium outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors" id="user-row-{{ $user->id }}">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <input type="text" class="name-edit hidden w-full bg-gray-50 border border-gray-300 rounded-lg p-2 text-xs font-medium focus:ring-2 focus:ring-blue-500" value="{{ $user->name }}" data-id="{{ $user->id }}" onchange="autoSaveUser({{ $user->id }})">
                            </td>
                            
                            {{-- Email --}}
                            <td class="px-6 py-4">
                                <span class="email-view text-xs text-gray-400 font-medium">{{ $user->email }}</span>
                                <input type="email" class="email-edit hidden w-full bg-gray-50 border border-gray-300 rounded-lg p-2 text-xs font-medium focus:ring-2 focus:ring-blue-500" value="{{ $user->email }}" data-id="{{ $user->id }}" onchange="autoSaveUser({{ $user->id }})">
                            </td>
                            
                            {{-- Role/Peran --}}
                            <td class="px-6 py-4">
                                <span class="role-view px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider {{ $user->role == 'admin' ? 'bg-blue-100 text-blue-600' : ($user->role == 'operator' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600') }}">
                                    {{ $user->role == 'admin' ? 'Administrator' : ($user->role == 'operator' ? 'Operator' : 'Pengguna') }}
                                </span>
                                <select class="role-edit hidden w-full bg-gray-50 border border-gray-300 rounded-lg p-2 text-xs font-medium focus:ring-2 focus:ring-blue-500" data-id="{{ $user->id }}" onchange="autoSaveUser({{ $user->id }})">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-3">
                                    {{-- Button Edit --}}
                                    <button type="button" onclick="toggleEditUserMode({{ $user->id }})" class="edit-user-btn text-blue-500 hover:text-blue-700 transition-colors text-sm font-medium">
                                        <i class="fas fa-edit"></i> Edit
                                    </button>
                                    
                                    {{-- Button Selesai (Hidden by default) --}}
                                    <button type="button" onclick="toggleEditUserMode({{ $user->id }})" class="done-user-btn hidden text-green-500 hover:text-green-700 transition-colors text-sm font-medium">
                                        <i class="fas fa-check"></i> Selesai
                                    </button>
                                    
                                    {{-- Button Hapus --}}
                                    @if(auth()->id() !== $user->id)
                                    <form action="{{ route('admin.deleteUser', $user->id) }}" method="POST" onsubmit="return confirm('Hapus pengguna ini?')" class="inline delete-user-form">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-red-400 hover:text-red-600 transition-colors text-sm font-medium">
                                            <i class="fas fa-trash"></i> Hapus
                                        </button>
...
// Toggle mode edit/view untuk User
function toggleEditUserMode(id) {
    ...
    if (isEditing) {
        ...
        row.querySelector('.email-view').classList.add('hidden');
        row.querySelector('.email-edit').classList.remove('hidden');
        row.querySelector('.role-view').classList.add('hidden');
        row.querySelector('.role-edit').classList.remove('hidden');
        
        // Toggle buttons
        row.querySelector('.edit-user-btn').classList.add('hidden');
        row.querySelector('.done-user-btn').classList.remove('hidden');
        
        const deleteForm = row.querySelector('.delete-user-form');
        if (deleteForm) {
            deleteForm.style.opacity = '0.3';
        ...
        row.querySelector('.email-view').classList.remove('hidden');
        row.querySelector('.email-edit').classList.add('hidden');
        row.querySelector('.role-view').classList.remove('hidden');
        row.querySelector('.role-edit').classList.add('hidden');
        
        // Toggle buttons
        row.querySelector('.edit-user-btn').classList.remove('hidden');
        row.querySelector('.done-user-btn').classList.add('hidden');
        
        const deleteForm = row.querySelector('.delete-user-form');
        if (deleteForm) {
            deleteForm.style.opacity = '1';
            deleteForm.style.pointerEvents = 'auto';
        }
...
// Auto-save saat ada perubahan User
function autoSaveUser(id) {
    ...
    .then(data => {
        if (data.success) {
            ...
            let roleBadgeClass = 'bg-gray-100 text-gray-600';
            let roleText = 'Pengguna';
            
            if (role === 'admin') {
                roleBadgeClass = 'bg-blue-100 text-blue-600';
                roleText = 'Administrator';
            } else if (role === 'operator') {
                roleBadgeClass = 'bg-green-100 text-green-600';
                roleText = 'Operator';
            }
            
            roleView.className = `role-view px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${roleBadgeClass}`;
            roleView.textContent = roleText;
            ...
            showNotification('Data pengguna tersimpan otomatis!', 'success');
        } else {
            showNotification('Gagal menyimpan: ' + (data.message || 'Terjadi kesalahan'), 'error');
            row.style.backgroundColor = originalBg;
        }
