...
@if(($currentTab ?? 'insight') == 'insight')
    ...
    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <form action="{{ route('operator.predict') }}" method="POST" id="mainForm" class="mt-6 pt-5 border-t border-slate-100">
            @csrf
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm" x-data="{ inputMode: 'single' }">
                ...
                {{-- Input Manual --}}
                <form x-show="inputMode === 'single'" action="{{ route('operator.storeData') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                ...
                {{-- Input CSV --}}
                <form x-show="inputMode === 'bulk'" action="{{ route('operator.storeData') }}" method="POST" enctype="multipart/form-data" class="space-y-4" x-cloak>
                    @csrf
                    <div>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                ...
                                <a href="{{ route('operator.downloadTemplate') }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-3 py-2 rounded-lg text-[8px] font-medium-clean uppercase tracking-widest hover:bg-blue-700 transition-colors">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <form action="{{ route('operator.predict') }}" method="POST" class="mb-6 pb-6 border-b border-slate-100">
                    @csrf
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100 text-[10px]">
                            @forelse($dataIssues ?? [] as $issue)
                                @php
                                    // Pastikan value selalu numerik (aman dari "Rp", koma, dll)
                                    $cleanValue = is_numeric($issue->value)
                                        ? (float) $issue->value
                                        : (float) preg_replace('/[^\d.]/', '', $issue->value);
                                @endphp

                                <tr class="bg-orange-50/20">
                                    <td class="px-6 py-3 font-medium-clean">
                                        {{ $issue->date }}
                                    </td>

                                    <td class="px-6 py-3">
                                        <span class="px-2 py-0.5 rounded-full text-[8px] font-medium-clean
                                            {{ $issue->issue === 'Outlier'
                                    ...
                                    <td class="px-6 py-3 font-medium-clean text-slate-700">
                                        Rp {{ number_format($cleanValue, 0, ',', '.') }}
                                    </td>
