...
<body class="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="hidden md:flex items-center space-x-1">
                    <a href="{{ route('laporan.komoditas.index') }}"
                       ...
                       <i class="fas fa-home mr-1.5 opacity-50"></i> {{ __('messages.beranda') }}
                    </a>

                    <a href="{{ route('dashboard') }}"
                       class="text-white/80 px-4 py-[22px] text-[11px] font-bold uppercase tracking-widest hover:text-white hover:bg-white/5 transition-standard
                       {{ request()->routeIs('dashboard') ? 'nav-link-active' : '' }}">
                       <i class="fas fa-chart-line mr-1.5 opacity-50"></i> {{ __('messages.analisis') }}
                    </a>
                    ...
                    @if(auth()->user()->isAdmin() || auth()->user()->isOperator())
                        <a href="{{ auth()->user()->isAdmin() ? route('admin.dashboard', ['tab' => 'manage']) : route('operator.dashboard', ['tab' => 'manage']) }}"
                           ...
                           <i class="fas fa-database mr-1.5 opacity-50"></i> {{ __('messages.manajemen_data') }}
                        </a>
                    @endif
                    ...
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.dashboard', ['tab' => 'users']) }}"
                           ...
                           <i class="fas fa-users mr-1.5 opacity-50"></i> {{ __('messages.manajemen_pengguna') }}
                        </a>
                    @endif
                </div>
                @endauth
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open"
                            ...
                            <!-- <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span id="theme-label" class="text-xs font-semibold text-gray-700 dark:text-gray-300"></span>
                                    </div>
                            ...
                            <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                                <p class="text-[9px] font-black uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">
                                    {{ __('messages.tampilan') }}
                                </p>
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <span id="theme-icon" class="text-sm">
                                            {{-- Icon akan diupdate via JS --}}
                                            ☀️
                                        </span>
                                        <span id="theme-label" 
    ...
    if (isDark) {
        label.innerText = label.getAttribute('data-label-dark');
        icon.innerText = '🌙';
        thumb.style.transform = 'translateX(1.25rem)'; // translate-x-5
    } else {
        label.innerText = label.getAttribute('data-label-light');
        icon.innerText = '☀️';
