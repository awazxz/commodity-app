...
v8.4 (Security: API Key Auth):
- Tambah @before_request middleware untuk validasi X-Flask-API-Key header.
- Semua endpoint /api/* dilindungi — hanya Laravel yang boleh akses.
- Endpoint yang dikecualikan: GET / (root info) dan GET /api/flask-health.
- Key dibaca dari environment variable FLASK_API_KEY.
...
CORS(app, resources={
    r"/api/*": {
        ...
        "allow_headers": ["Content-Type", "Authorization", "X-Flask-API-Key"],
    }
})
...
# ═══════════════════════════════════════════════════════════════
# v8.4: API KEY MIDDLEWARE
# ═══════════════════════════════════════════════════════════════

# Endpoint yang TIDAK perlu API key (boleh diakses bebas)
_PUBLIC_ENDPOINTS = {
    '/',               # root info
    '/api/flask-health',
    '/api/health',
...
def require_api_key():
    """
    Validasi X-Flask-API-Key header pada setiap request ke /api/*.
    Endpoint di _PUBLIC_ENDPOINTS dikecualikan.
    OPTIONS request dikecualikan (CORS preflight).
    """
    # Lewati OPTIONS (CORS preflight)
    ...
    # Hanya proteksi path yang dimulai dengan /api/
    if not request.path.startswith('/api/'):
        return

    flask_api_key = os.getenv('FLASK_API_KEY', '').strip()

    # Jika FLASK_API_KEY belum di-set di .env, tolak semua request dengan pesan jelas
    if not flask_api_key:
        return jsonify({
            'success': False,
            'message': 'Server configuration error: FLASK_API_KEY not set.',
        }), 500

    incoming_key = request.headers.get('X-Flask-API-Key', '').strip()

    if not incoming_key:
        return jsonify({
            'success': False,
            'message': 'Unauthorized: X-Flask-API-Key header missing.',
        }), 401

    # Gunakan compare_digest untuk mencegah timing attack
    import hmac
    if not hmac.compare_digest(incoming_key, flask_api_key):
        return jsonify({
            'success': False,
...
def predict_forecast_advanced():
    ...
    v8.4: Dilindungi API key (X-Flask-API-Key header).
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("  COMMODITY PRICE FORECASTING API  v8.4.0")
    print("=" * 60)
    print(f"   PORT      : {PORT}")
    print(f"   DEBUG     : {DEBUG}")
    print(f"   URL       : http://127.0.0.1:{PORT}")
    print(f"   SCHEDULER : aktif (retrain otomatis jam 02:00)")
    print(f"   v8.4      : API key auth aktif (X-Flask-API-Key)")
    ...
    # Cek FLASK_API_KEY saat startup
    if not os.getenv('FLASK_API_KEY', '').strip():
        print("\n  ⚠️  WARNING: FLASK_API_KEY belum di-set di .env!")
        print("     Semua request ke /api/* akan ditolak.")
        print("     Tambahkan FLASK_API_KEY ke file .env Flask.\n")
